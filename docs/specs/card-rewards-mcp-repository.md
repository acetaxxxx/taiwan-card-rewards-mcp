...
- Every Aion descriptor pins an immutable package version, for example
  `taiwan-card-rewards-mcp@0.8.0`; never use `latest`, a moving tag, or an
...
```json
{
  ...
  "transport": {
    "type": "stdio",
    "command": "npx",
    "args": ["--yes", "taiwan-card-rewards-mcp@0.8.0", "--data-dir", "/srv/aion/users/<user>/taiwan-card-rewards-mcp"]
  },
  "enabled": true
}
```
