...
- Every Aion descriptor pins an immutable package version, for example
  `taiwan-card-rewards-mcp@0.10.1`; never use `latest`, a moving tag, or an
...
```json
{
  "name": "taiwan_card_rewards_mcp",
  "transport": {
    "type": "stdio",
    "command": "npx",
    "args": ["--yes", "taiwan-card-rewards-mcp@0.10.1", "--data-dir", "/srv/aion/users/<user>/taiwan-card-rewards-mcp"]
  },
  "enabled": true
}
```
