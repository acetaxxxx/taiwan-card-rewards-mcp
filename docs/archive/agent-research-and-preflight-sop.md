...
**文件狀態**：歷史封存文件 (Archived Historical SOP - Non-normative)
**歷史適用範圍**：歷史 20-tool MCP contract（現行 28-tool 合約請見 `references/mcp-tools.md`）
**語言**：繁體中文
...
## 2. Canonical 20-tool MCP 合約體系（歷史封存版本）

本節記錄歷史 20-tool baseline；現行伺服器公開 28 項標準工具，完整清單、closed schema 與逐項用途說明一律以 [`mcp-tools.md`](../taiwan-card-rewards-skill/references/mcp-tools.md) 為準，本文件不重複維護第二份清單以避免走漏更新。
