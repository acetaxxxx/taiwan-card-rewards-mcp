...
### REQ-6: 外幣匯率自然輸入（免 Agent 計算 `ratePpm`）
- **6.1** 使用者或 Agent 傳入即時牌告或自訂匯率快照（`FxSnapshot`）時，全面支援自然小數匯率（如 JPY/TWD 直接輸入 `rate: 0.215`，USD/TWD 直接輸入 `rate: 32.5`），嚴禁要求 Agent 自行手動計算或乘以一百萬換算為百萬分率（`ratePpm: 215000`）。
- **6.2** 系統底層自動將自然匯率轉換為精確的整數 `ratePpm`（`Math.round(rate * 1_000_000)`），兼顧 Agent 易用性與底層嚴格整數運算。
- **6.3** 具備強韌容錯機制：若 Agent 誤將小數匯率填入 `ratePpm`（例如 `ratePpm: 0.215`），系統自動識別為自然小數並自動放大為整數百萬分率，避免靜默換算錯誤。
- **6.4** 完整向後相容傳統整數 `ratePpm` 輸入，維持合約穩健度。
