...
```bash
npx --yes github:acetaxxxx/taiwan-card-rewards-mcp#v0.10.1 \
...
```json
{
  "mcpServers": {
    "taiwan_card_rewards_mcp": {
      ...
      "args": [
        "--yes",
        "github:acetaxxxx/taiwan-card-rewards-mcp#v0.10.1",
        "--data-dir",
...
- [ ] 2. 伺服器握手 (Handshake Validation)
      ...
      - 驗證 `serverInfo.name` 為 `taiwan_card_rewards_mcp`，`serverInfo.version` 為 `0.10.1` (或更新相容版本)。
...
2. **資料目錄備份**：
   ...
   ```
3. **更新版本宣告**：將啟動命令或設定檔中的版本 tag 指向新版本（例如 `#v0.10.1`）。
