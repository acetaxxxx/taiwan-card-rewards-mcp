...
這個 bundle 提供 Agent 可直接載入的 canonical Skill、20-tool MCP reference、workflows 與端到端呼叫範例。MCP 是零網路的 deterministic 計算核心與 user-scoped durable ledger；官方頁面、PDF、圖片、OCR 與匯率研究由 Agent/UI 在 MCP 外完成。
...
安裝時複製整個 `taiwan-card-rewards-skill/` 目錄，不要只複製 `SKILL.md`。載入 [`SKILL.md`](SKILL.md) 後，Agent 依需求漸進載入 reference/workflow；不要複製猜測 schema。可直接交給 AI agent 執行的安裝指令見 repo 根目錄 [`INSTALL_WITH_AI.md`](../../../INSTALL_WITH_AI.md)。
...
├── README.md                                 # 安裝與目錄導覽
├── references/
│   ├── low-reasoning-playbook.md              # Gemini Flash、Luna 等輕量模型三層指引
│   ├── mcp-tools.md                           # canonical 20-tool contract
...
文件變更後，從 repo 根目錄執行 `npm run typecheck`、`npm run build`、`npm test`、`git diff --check`；再檢查 Markdown links 與 canonical 20-tool/name grep。此 bundle 不代表真實銀行產品、sidecar 或 staging 已驗證。
