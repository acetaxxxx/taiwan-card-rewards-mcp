...
歡迎使用台灣信用卡回饋分析與推薦技能包（Taiwan Card Rewards Skill Bundle）。本套件提供專為大型語言模型與代理人（AI Agents）設計的規範文件、流程 SOP 與模板，搭配 `taiwan-card-rewards-mcp` (v0.9.0, 13 Tools) 實現高精度、可解釋且安全的信用卡回饋試算與消費管理。
...
│   ├── mcp-tools-v0.9.0.md                  # 13 項公開 MCP 工具簽名、參數與錯誤碼
...
│   └── payment-route-and-fx.md              # 支付通道上下文與外幣匯率 PPM SOP
...
└── examples/                                # [範例] 完整端到端實例
    ...
    ├── merchant-ambiguity-recovery.md       # 商家模糊消歧義修復流程
    ├── stale-fx-recovery.md                 # 外幣即時匯率查核與快照補齊
