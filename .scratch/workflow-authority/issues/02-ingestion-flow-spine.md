...
- [ ] Create 接受可正規化、非敏感的 source scope；同一 `(ownerUser, source scope)` 的 active create 會回既有 flow，不會堆積重複 drafts。
- [ ] Source scope lock 不以 card ID 作全域互斥；一份來源可涵蓋多產品，Held Card 註冊不依賴 ingestion complete。
...
- [ ] Draft 保存 `lastActivityAt`、`expiresAt` 與 terminal `expired` 狀態；只有 MCP 接受的 expected action 會更新活動時間。
- [ ] Startup 與 mutating flow operations 會 sweep 過期 drafts；另有可由 operator/scheduler 呼叫的 maintenance sweep，使用 injectable clock 做 deterministic test。
- [ ] Expiry 只刪除 incomplete candidate artifacts 與未被 completed work 參照的 payload，保留 tenant-scoped minimal tombstone；active rules、完成 proof 與歷史資料不可被 sweep。
- [ ] Expired draft 必須以新 revision 建立，不能直接恢復；相同 source scope 的新 flow 不受已過 retention tombstone 阻擋。
