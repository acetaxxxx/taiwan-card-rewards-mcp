...
export const mcpInstructions = [
  ...
  'The Agent or UI must obtain and parse bank pages, images, or PDFs before calling this server. A user-confirmed private offer may be activated without official verification, but must retain provenance and is never presented as issuer-verified.',
...
const closed = (properties: Record<string, unknown>, required: readonly string[] = []): Record<string, unknown> => ({ type: 'object', additionalProperties: true, properties, ...(required.length ? { required } : {}) });
const string = { type: 'string' };
const date = { type: 'string', format: 'date-time' };
const money = closed({
  amount: { type: 'number', minimum: 0, description: 'Natural currency amount (e.g. 4.8 for USD, 20 for TWD, 30000 for JPY). Internal minor-unit scaling is handled automatically.' },
  value: { type: 'number', minimum: 0, description: 'Alias for natural currency amount.' },
  amountMinor: { type: 'number', minimum: 0, description: 'Minor currency units (e.g. cents). Supported for backwards compatibility.' },
  currency: string,
}, ['currency']);
...
const snapshot = closed({ id: string, ownerUser: string, url: { type: 'string', format: 'uri' }, fetchedAt: date, contentHash: string, parserVersion: string, validFrom: date, validTo: date, excerpt: string, verified: { type: 'boolean' }, sourceType: { type: 'string', enum: ['official', 'user_input'] }, provenance }, ['id', 'fetchedAt', 'contentHash', 'parserVersion']);
...
const predicate: Record<string, unknown> = closed({ op: { type: 'string', enum: ['AND', 'OR', 'NOT', 'EQUALS', 'MATCH_ALLOWLIST'] }, field: string, value: {}, rules: { type: 'array', items: { type: 'object', additionalProperties: true }, maxItems: 64 }, rule: { type: 'object', additionalProperties: true } }, ['op']);
...
const paymentRouteSelector = closed({
  paymentServices: { type: 'array', items: string, maxItems: 64 },
  paymentServiceAllowlist: { type: 'array', items: string, maxItems: 64 },
  acceptanceNetworks: { type: 'array', items: string, maxItems: 64 },
  acceptanceNetworkAllowlist: { type: 'array', items: string, maxItems: 64 },
  fundingKinds: { type: 'array', items: { type: 'string', enum: ['credit_card', 'account', 'cash'] }, maxItems: 3 },
  nodeRoles: { type: 'array', items: { type: 'string', enum: ['funding_source', 'wallet_balance', 'payment_service', 'acceptance_network', 'merchant'] }, maxItems: 8 },
  transitions: { type: 'array', items: { type: 'string', enum: ['card_authorization', 'account_debit', 'wallet_top_up', 'wallet_debit', 'service_to_acceptance', 'merchant_settlement', 'direct_settlement', 'split_tender'] }, maxItems: 16 },
  excludedTransitions: { type: 'array', items: { type: 'string', enum: ['card_authorization', 'account_debit', 'wallet_top_up', 'wallet_debit', 'service_to_acceptance', 'merchant_settlement', 'direct_settlement', 'split_tender'] }, maxItems: 16 },
  validFrom: date,
  validTo: date,
});
...
const fundingInstrument = closed({ kind: { type: 'string', enum: ['credit_card', 'account', 'cash'] }, cardId: string, accountId: string, subtype: { type: 'string', enum: ['linked_bank_account', 'wallet_balance', 'foreign_currency_account'] } }, ['kind']);
const paymentRoute = closed({ id: string, status: { type: 'string', enum: ['candidate', 'active', 'stale', 'conflict', 'needs_review', 'failed'] }, layers: { type: 'array', items: closed({ kind: { type: 'string', enum: ['merchant_loyalty', 'merchant_acceptance', 'consumer_app', 'payment_provider', 'wallet', 'interoperability_scheme', 'intermediate_provider', 'card_network', 'card_issuer'] }, providerId: string, appId: string, paymentMethod: string, displayName: string, evidenceIds: { type: 'array', items: string, maxItems: 32 } }) }, funding: fundingInstrument, sourceUrl: string, sourceSnapshotId: string, contentHash: string, observedAt: date, validFrom: date, validTo: date, authority: { type: 'string', enum: ['issuer', 'network', 'wallet', 'merchant', 'secondary', 'community', 'user'] }, confidence: { type: 'string', enum: ['high', 'medium', 'low'] }, confirmation: closed({ confirmedAt: date, confirmedBy: string }, ['confirmedAt', 'confirmedBy']), failure: closed({ failedAt: date, failedBy: string, reason: string }, ['failedAt', 'failedBy', 'reason']), evidenceIds: { type: 'array', items: string, maxItems: 64 }, nodes: { type: 'array', items: paymentRouteNode, maxItems: 128 }, edges: { type: 'array', items: paymentRouteEdge, maxItems: 256 }, idempotencyKey: string }, ['layers', 'funding', 'observedAt', 'idempotencyKey']);
...
const transaction = closed({ idempotencyKey: string, routeId: string, cardId: string, funding: fundingInstrument, kind: { type: 'string', enum: ['purchase', 'refund'] }, mode: { type: 'string', enum: ['planned', 'actual'] }, merchant: string, mcc: string, country: string, channel: string, paymentMethod: string, occurredAt: date, recordedAt: date, amount: money, fx, refundOfId: string, originalRewardMinor: { type: 'integer', minimum: 0 }, route, routeContext, settlementAmount: money }, ['kind', 'mode', 'occurredAt', 'amount']);
const paymentEvent = closed({ id: string, kind: { type: 'string', enum: ['top_up', 'purchase', 'refund', 'reversal', 'reward_issuance', 'reward_redemption'] }, amount: money, occurredAt: date, funding: fundingInstrument, cardId: string, routeId: string, channel: string, paymentMethod: string, relations: closed({ funded_by: { type: 'array', items: string, maxItems: 16 }, caused_by: { type: 'array', items: string, maxItems: 16 }, refunds: { type: 'array', items: string, maxItems: 16 } }), fx }, ['id', 'kind', 'amount', 'occurredAt', 'funding']);
...
const fxArrayOrSingle = { oneOf: [fx, { type: 'array', items: fx, maxItems: 128 }] };
const recommendationSupplementalFacts = closed({ merchant: closed({ canonicalId: string, canonicalNameZhHant: string, country: string, market: string }, ['canonicalId']), amount: money, transaction: closed({ amount: money, country: string, market: string, channel: string, paymentMethod: string, occurredAt: date }), fx: fxArrayOrSingle, routeFacts: { type: 'array', items: closed({ routeId: string, edgeId: string, fx }, ['routeId', 'fx']), maxItems: 128 }, eligibilityFacts: { type: 'array', items: eligibilityFact, maxItems: 128 }, benefitEvidence: { type: 'array', items: closed({ evidenceId: string, observedAt: date, sourceUrl: { type: 'string', format: 'uri' }, contentHash: string, scope: string }, ['evidenceId', 'observedAt']), maxItems: 128 }, childFlowId: string, resumedFlowId: string });
const recommendationIntent = closed({ merchant: { oneOf: [string, closed({ canonicalId: string, canonicalNameZhHant: string, rawStatement: string, name: string, market: string, country: string })] }, amount: { oneOf: [{ type: 'number', minimum: 0, description: 'Preferred: natural transaction amount at top level (e.g. 4.8, 20, 30000). Pair with top-level currency field. Defaults to TWD if currency omitted.' }, money] }, currency: { type: 'string', description: 'ISO 4217 currency code for the top-level amount. Defaults to "TWD" if omitted.' }, fx: fxArrayOrSingle, routeFacts: { type: 'array', items: closed({ routeId: string, edgeId: string, fx }, ['routeId', 'fx']), maxItems: 128 }, eligibilityFacts: { type: 'array', items: eligibilityFact, maxItems: 128 }, supplementalFacts: recommendationSupplementalFacts, expectedResultVersion: string, childFlowId: string, resumedFlowId: string, country: string, market: string, channel: string, paymentMethod: string, occurredAt: date, cardIds: { type: 'array', items: string, maxItems: 128 }, routeIds: { type: 'array', items: string, maxItems: 128 }, limit: { type: 'integer', minimum: 1, maximum: 128 }, page: { type: 'integer', minimum: 1 }, cursor: string, resultVersion: string }, ['merchant']);
...
const userFacts = { type: 'object', additionalProperties: true, patternProperties: { '^(user\\.)?[A-Za-z][A-Za-z0-9_.]{0,127}$': { oneOf: [string, { type: 'number' }, { type: 'boolean' }, { type: 'array', items: string }] } } };
...
const context = closed({ now: date, usageByKey: { type: 'object', additionalProperties: true, patternProperties: { '^[A-Za-z0-9][A-Za-z0-9_:-]{0,127}$': money } }, sourceSnapshots: { type: 'object', additionalProperties: true, patternProperties: { '^[A-Za-z0-9][A-Za-z0-9_:-]{0,127}$': snapshot } }, capPools: { type: 'array', items: capPool }, userConfirmed: { type: 'boolean' }, heldCards: { type: 'array', items: heldCard }, eligibilityFacts: { type: 'array', items: eligibilityFact }, userFacts, paymentEvents: closed({ target: paymentEvent, sourceEvents: { type: 'array', items: paymentEvent, maxItems: 16 } }, ['target', 'sourceEvents']) });
...
const ingestionSourceScope = closed({ kind: { type: 'string', enum: ['official_url', 'offer_family'] }, value: { type: 'string', minLength: 1, maxLength: 512 } }, ['kind', 'value']);
const ingestionParentContinuation = closed({ intentFingerprint: string, parentResultVersion: string, childFlowId: string, sourceScope: ingestionSourceScope, ruleFamily: string }, ['intentFingerprint', 'parentResultVersion']);
const ingestionSourceCapture = closed({ sourceType: { type: 'string', enum: ['official', 'user_input'] }, url: { type: 'string', format: 'uri' }, description: string, retrievedAt: date, contentHash: string, artifactRef: string, submitter: string, submittedAt: date }, ['sourceType', 'retrievedAt', 'contentHash', 'artifactRef', 'submitter', 'submittedAt']);
const ingestionManifestLeaf = closed({ id: string, kind: { type: 'string', enum: ['benefit', 'exclusion'] }, summary: string, evidenceLocator: string, dependsOn: { type: 'array', items: string, maxItems: 128 } }, ['id', 'kind', 'summary', 'evidenceLocator', 'dependsOn']);
const ingestionLocalExclusion = closed({ scope: { const: 'benefit' }, predicate, evidenceRefs: { type: 'array', items: string, minItems: 1, maxItems: 32 } }, ['scope', 'predicate', 'evidenceRefs']);
const ingestionExclusionScope = { oneOf: [closed({ kind: { const: 'all_benefits' } }, ['kind']), closed({ kind: { const: 'benefit_ids' }, benefitIds: { type: 'array', items: string, minItems: 1, maxItems: 128 } }, ['kind', 'benefitIds'])] };
const ingestionExclusionLeaf = closed({ flowId: string, actionId: string, expectedRevision: { type: 'integer', minimum: 1 }, idempotencyKey: string, leafId: string, target: { type: 'string', enum: ['merchant', 'transaction_fact', 'payment_route', 'payment_method'] }, scope: ingestionExclusionScope, predicate, evidenceRefs: { type: 'array', items: string, minItems: 1, maxItems: 32 }, disposition: { type: 'string', enum: ['materialized', 'ignored', 'superseded'] }, reason: string }, ['flowId', 'actionId', 'expectedRevision', 'idempotencyKey', 'leafId', 'target', 'scope', 'predicate', 'evidenceRefs']);
const merchantReference = closed({ rawQuery: string, canonicalId: string, country: string, market: string, mcc: string, channel: string, candidate: closed({ canonicalNameZhHant: string, canonicalNameLocale: { const: 'zh-Hant-TW' }, officialAliases: { type: 'array', items: string, maxItems: 128 }, operatingMarkets: { type: 'array', items: string, maxItems: 64 }, mccs: { type: 'array', items: string, maxItems: 64 }, channels: { type: 'array', items: { type: 'string', enum: ['in_store', 'online'] }, maxItems: 2 }, provenance: merchantIdentity.properties && (merchantIdentity.properties as Record<string, unknown>).provenance }, ['canonicalNameZhHant', 'canonicalNameLocale', 'provenance']) });
const ingestionBenefitLeaf = closed({ flowId: string, actionId: string, expectedRevision: { type: 'integer', minimum: 1 }, idempotencyKey: string, leafId: string, disposition: { type: 'string', enum: ['materialized', 'ignored', 'superseded'] }, reason: string, offer: closed({ snapshot, rule, capPools: { type: 'array', items: capPool, maxItems: 128 }, merchant: merchantIdentity }, ['snapshot', 'rule']), merchantRefs: { type: 'array', items: merchantReference, maxItems: 32 }, evidenceRefs: { type: 'array', items: string, minItems: 1, maxItems: 32 }, localExclusions: { type: 'array', items: ingestionLocalExclusion, maxItems: 32 } }, ['flowId', 'actionId', 'expectedRevision', 'idempotencyKey', 'leafId', 'evidenceRefs']);
const ingestionFinalize = closed({ flowId: string, actionId: string, expectedRevision: { type: 'integer', minimum: 1 } }, ['flowId', 'actionId', 'expectedRevision']);
const ingestionManifestCorrection = closed({ flowId: string, actionId: string, expectedRevision: { type: 'integer', minimum: 1 }, idempotencyKey: string, manifest: { type: 'array', items: ingestionManifestLeaf, minItems: 1, maxItems: 128 } }, ['flowId', 'actionId', 'expectedRevision', 'idempotencyKey', 'manifest']);

export const mcpTools: readonly McpToolContract[] = [
  { name: 'create_ingestion', description: 'Create or resume one authenticated, source-scoped ingestion draft. This flow does not register a card and accepts no source payload yet.', readOnly: false, inputSchema: closed({ sourceScope: ingestionSourceScope, idempotencyKey: string, parentContinuation: ingestionParentContinuation }, ['sourceScope', 'idempotencyKey']), failClosedErrors: ['UNAUTHENTICATED', 'IDEMPOTENCY_CONFLICT', 'STORE_UNAVAILABLE'] },
  { name: 'get_ingestion', description: 'Inspect the authenticated user’s ingestion draft and its one server-owned next action.', readOnly: true, inputSchema: closed({ flowId: string }, ['flowId']), failClosedErrors: ['UNAUTHENTICATED', 'FLOW_NOT_FOUND', 'STORE_UNAVAILABLE'] },
  { name: 'submit_ingestion_source', description: 'Submit exactly one immutable source capture for the current ingestion action. The server does not fetch URLs or treat agent claims as authority.', readOnly: false, inputSchema: closed({ flowId: string, actionId: string, expectedRevision: { type: 'integer', minimum: 1 }, sourceCapture: ingestionSourceCapture }, ['flowId', 'actionId', 'expectedRevision', 'sourceCapture']), failClosedErrors: ['UNAUTHENTICATED', 'STALE_REVISION', 'INVALID_FLOW_ACTION', 'SOURCE_SCOPE_CONFLICT', 'STORE_UNAVAILABLE'] },
  { name: 'submit_ingestion_manifest', description: 'Submit the complete closed source manifest for the current ingestion action. The server rejects cycles and incomplete dependency graphs.', readOnly: false, inputSchema: closed({ flowId: string, actionId: string, expectedRevision: { type: 'integer', minimum: 1 }, manifest: { type: 'array', items: ingestionManifestLeaf, minItems: 1, maxItems: 128 } }, ['flowId', 'actionId', 'expectedRevision', 'manifest']), failClosedErrors: ['UNAUTHENTICATED', 'STALE_REVISION', 'INVALID_FLOW_ACTION', 'INVALID_INPUT', 'STORE_UNAVAILABLE'] },
  { name: 'correct_ingestion_manifest', description: 'Create a server-owned manifest revision while preserving superseded manifest lineage.', readOnly: false, inputSchema: ingestionManifestCorrection, failClosedErrors: ['UNAUTHENTICATED', 'STALE_REVISION', 'INVALID_FLOW_ACTION', 'IDEMPOTENCY_CONFLICT', 'INVALID_INPUT', 'STORE_UNAVAILABLE'] },
  { name: 'submit_benefit_leaf', description: 'Validate one server-selected benefit leaf, materialize one candidate rule through the existing Offer Catalog adapter, and advance manifest coverage. Candidate rules remain invisible until finalization.', readOnly: false, inputSchema: ingestionBenefitLeaf, failClosedErrors: ['UNAUTHENTICATED', 'STALE_REVISION', 'INVALID_FLOW_ACTION', 'SOURCE_SCOPE_CONFLICT', 'NEEDS_REVIEW', 'INVALID_OFFER', 'IDEMPOTENCY_CONFLICT', 'STORE_UNAVAILABLE'] },
  { name: 'submit_exclusion_leaf', description: 'Validate one server-selected shared exclusion with explicit scope and evidence. It creates only a candidate artifact; ignored exclusions require a reason.', readOnly: false, inputSchema: ingestionExclusionLeaf, failClosedErrors: ['UNAUTHENTICATED', 'STALE_REVISION', 'INVALID_FLOW_ACTION', 'NEEDS_REVIEW', 'IDEMPOTENCY_CONFLICT', 'STORE_UNAVAILABLE'] },
  { name: 'finalize_ingestion', description: 'Atomically revalidate a complete ingestion manifest, activate only official verified candidate rules, and return a durable completion proof. Exact retries return the same proof.', readOnly: false, inputSchema: ingestionFinalize, failClosedErrors: ['UNAUTHENTICATED', 'STALE_REVISION', 'INVALID_FLOW_ACTION', 'NEEDS_REVIEW', 'STORE_UNAVAILABLE'] },
  ...
  { name: 'recommend', description: 'Return bounded merchant-first recommendations across registered cards and evidence-backed payment paths in one unified, ranked candidate list.', readOnly: true, inputSchema: recommendationIntent, failClosedErrors: ['INVALID_INPUT', 'PAYLOAD_TOO_LARGE', 'INSUFFICIENT_FACTS', 'NEEDS_REVIEW', 'STALE'] },
  ...
  { name: 'list_transactions', description: 'List recorded actual transactions within an optional time range, sorted chronologically with stable pagination and summary/detail projections.', readOnly: true, inputSchema: closed({ startDate: date, endDate: date, timeBasis: { type: 'string', enum: ['occurred_at', 'recorded_at'] }, fundingKind: { type: 'string', enum: ['credit_card', 'account', 'cash'] }, cardId: string, ...listPage, projection: { type: 'string', enum: ['summary', 'detail'] } }), failClosedErrors: ['INVALID_INPUT', 'PAYLOAD_TOO_LARGE', 'STORE_UNAVAILABLE'] },
  ...
  { name: 'remaining_caps', description: 'Report bounded remaining cap projections for currently usable rewards. Set includeHistorical to true only when auditing inactive, expired, superseded, or otherwise unavailable rule history.', readOnly: true, inputSchema: closed({ cardId: string, asOf: date, includeHistorical: { type: 'boolean', default: false }, ...page, ...projection }, ['cardId']), failClosedErrors: ['INVALID_INPUT', 'PAYLOAD_TOO_LARGE', 'STORE_UNAVAILABLE'] },
  ...
  { name: 'search_active_offers', description: 'Search active, in-window public verified and current-user user-confirmed offers with bounded 1-based pagination; read-only and never activates rules.', readOnly: true, inputSchema: closed({ rawQuery: { type: 'string', maxLength: 128 }, cardId: string, canonicalMerchantId: string, country: string, market: string, mcc: string, channel: string, asOf: date, ...page, projection: { type: 'string', enum: ['summary', 'detail'] } }), failClosedErrors: ['INVALID_INPUT', 'PAYLOAD_TOO_LARGE', 'INSUFFICIENT_FACTS'] },
];
