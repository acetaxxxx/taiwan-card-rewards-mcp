...
function validateIngestionFlow(value: unknown): IngestionFlowRecord {
  ...
  keys(item, ['id', 'ownerUser', 'sourceScope', 'revision', 'status', 'idempotencyKey', 'createdAt', 'lastActivityAt', 'expiresAt', 'sourceCapture', 'manifest', 'benefitArtifacts'], 'ingestion flow');
...
export function validateIngestionBenefitLeaf(value: unknown): IngestionBenefitLeafSubmission {
  const item = object(value, 'benefit leaf submission'); keys(item, ['flowId', 'actionId', 'expectedRevision', 'idempotencyKey', 'leafId', 'offer', 'evidenceRefs', 'localExclusions'], 'benefit leaf submission');
  ...
  const offer = object(item.offer, 'benefit leaf offer'); keys(offer, ['snapshot', 'rule', 'capPools', 'merchant'], 'benefit leaf offer');
  ...
  const normalizedOffer: IngestionBenefitLeafSubmission['offer'] = { snapshot: validateSnapshot(offer.snapshot), rule: validateRule(offer.rule) };
  if (capPools !== undefined) normalizedOffer.capPools = capPools;
  if (offer.merchant !== undefined) normalizedOffer.merchant = offer.merchant as NonNullable<IngestionBenefitLeafSubmission['offer']['merchant']>;
  return { flowId: requiredString(item.flowId, 'benefit leaf flowId', true), actionId: requiredString(item.actionId, 'benefit leaf actionId', true), expectedRevision: item.expectedRevision as number, idempotencyKey: requiredString(item.idempotencyKey, 'benefit leaf idempotencyKey', true), leafId: requiredString(item.leafId, 'benefit leaf leafId', true), offer: normalizedOffer, evidenceRefs: item.evidenceRefs.map((v, i) => requiredString(v, `benefit leaf evidenceRefs[${i}]`)), ...(localExclusions ? { localExclusions } : {}) };
}

export function validateIngestionManifest(value: unknown): readonly IngestionManifestLeaf[] {
  ...
  const leaves = value.map((entry, index) => { const item = object(entry, `manifest[${index}]`); keys(item, ['id', 'kind', 'summary', 'evidenceLocator', 'dependsOn'], `manifest[${index}]`); const kind = requiredString(item.kind, `manifest[${index}].kind`); if (kind !== 'benefit' && kind !== 'exclusion') throw new RewardServiceError('INVALID_INPUT', `manifest[${index}].kind is invalid`); if (!Array.isArray(item.dependsOn)) throw new RewardServiceError('INVALID_INPUT', `manifest[${index}].dependsOn must be an array`); return { id: requiredString(item.id, `manifest[${index}].id`, true), kind, summary: requiredString(item.summary, `manifest[${index}].summary`), evidenceLocator: requiredString(item.evidenceLocator, `manifest[${index}].evidenceLocator`), dependsOn: item.dependsOn.map((id, i) => requiredString(id, `manifest[${index}].dependsOn[${i}]`, true)) } as IngestionManifestLeaf; });
