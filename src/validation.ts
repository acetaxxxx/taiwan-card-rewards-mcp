...
export function validateFxPolicy(value: unknown): FxPolicyRecord {
  ...
  keys(item, ['id', 'policyKey', 'version', 'scope', 'conversionOwner', 'rateType', 'rateDirection', 'conversionTiming', 'feeBasis', 'markupBasis', 'sourceUrl', 'evidenceId', 'observedAt', 'validFrom', 'validTo', 'idempotencyKey', 'ownerUser'], 'fx policy');
  ...
  return { id: requiredString(item.id, 'fx policy.id', true), policyKey: requiredString(item.policyKey, 'fx policy.policyKey', true), version: requiredString(item.version, 'fx policy.version', true), scope: { kind: kind as FxPolicyRecord['scope']['kind'], ...(scope.cardId === undefined ? {} : { cardId: requiredString(scope.cardId, 'fx policy.scope.cardId', true) }), ...(scope.issuer === undefined ? {} : { issuer: requiredString(scope.issuer, 'fx policy.scope.issuer') }), ...(scope.routeId === undefined ? {} : { routeId: requiredString(scope.routeId, 'fx policy.scope.routeId', true) }), ...(scope.edgeId === undefined ? {} : { edgeId: requiredString(scope.edgeId, 'fx policy.scope.edgeId', true) }) }, conversionOwner: owner as FxPolicyRecord['conversionOwner'], rateType: rateType as FxPolicyRecord['rateType'], rateDirection: item.rateDirection === 'base_to_quote' ? 'base_to_quote' : (() => { throw new RewardServiceError('INVALID_INPUT', 'fx policy rateDirection is invalid'); })(), conversionTiming: timing as FxPolicyRecord['conversionTiming'], ...(item.feeBasis === undefined ? {} : { feeBasis: requiredString(item.feeBasis, 'fx policy.feeBasis') }), ...(item.markupBasis === undefined ? {} : { markupBasis: requiredString(item.markupBasis, 'fx policy.markupBasis') }), ...(item.sourceUrl === undefined ? {} : { sourceUrl: requiredString(item.sourceUrl, 'fx policy.sourceUrl') }), evidenceId: requiredString(item.evidenceId, 'fx policy.evidenceId', true), observedAt: iso(item.observedAt, 'fx policy.observedAt'), ...(item.validFrom === undefined ? {} : { validFrom: iso(item.validFrom, 'fx policy.validFrom') }), ...(item.validTo === undefined ? {} : { validTo: iso(item.validTo, 'fx policy.validTo') }), idempotencyKey: requiredString(item.idempotencyKey, 'fx policy.idempotencyKey', true), ...(item.ownerUser === undefined ? {} : { ownerUser: requiredString(item.ownerUser, 'fx policy.ownerUser', true) }) };
}
