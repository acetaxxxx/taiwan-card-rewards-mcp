...
function validatePaymentRouteContext(value: unknown): PaymentRouteContext {
  ...
  keys(item, ['merchantId', 'walletProviderId', 'paymentMethod', 'intermediateProviderId', 'cardNetwork', 'issuer', 'fundingSource', 'transactionCurrency', 'settlementCurrency', 'billingCurrency', 'conversionOwner', 'rateType', 'conversionTiming', 'foreignTransactionFee', 'markup', 'serviceFee', 'dcc'], 'transaction.routeContext');
  ...
  if (owner !== undefined && !['merchant', 'wallet', 'payment_provider', 'card_network', 'issuer', 'unknown'].includes(owner)) throw new RewardServiceError('INVALID_INPUT', 'transaction.routeContext.conversionOwner is invalid');
...
export function validatePaymentRouteRecord(value: unknown): PaymentRouteRecord {
  ...
  keys(item, ['id', 'status', 'layers', 'funding', 'sourceUrl', 'sourceSnapshotId', 'contentHash', 'observedAt', 'validFrom', 'validTo', 'authority', 'confidence', 'confirmation', 'idempotencyKey', 'ownerUser'], 'payment route');
  const layers = Array.isArray(item.layers) ? item.layers.map((entry) => { const layer = object(entry, 'payment route layer'); keys(layer, ['kind', 'providerId', 'appId', 'paymentMethod', 'displayName'], 'payment route layer'); const kind = requiredString(layer.kind, 'payment route layer.kind'); if (!['merchant_loyalty', 'payment_provider', 'wallet', 'intermediate_provider', 'card_network', 'card_issuer'].includes(kind)) throw new RewardServiceError('INVALID_INPUT', 'payment route layer kind is invalid'); return { kind: kind as PaymentRouteRecord['layers'][number]['kind'], ...(layer.providerId === undefined ? {} : { providerId: requiredString(layer.providerId, 'payment route layer.providerId') }), ...(layer.appId === undefined ? {} : { appId: requiredString(layer.appId, 'payment route layer.appId') }), ...(layer.paymentMethod === undefined ? {} : { paymentMethod: requiredString(layer.paymentMethod, 'payment route layer.paymentMethod') }), ...(layer.displayName === undefined ? {} : { displayName: requiredString(layer.displayName, 'payment route layer.displayName') }) }; }) : (() => { throw new RewardServiceError('INVALID_INPUT', 'payment route layers must be an array'); })();
  const funding = object(item.funding, 'payment route funding'); keys(funding, ['kind', 'cardId', 'subtype'], 'payment route funding'); const fundingKind = requiredString(funding.kind, 'payment route funding.kind');
  const subtype = funding.subtype === undefined ? undefined : requiredString(funding.subtype, 'payment route funding.subtype');
  const normalizedFunding = fundingKind === 'credit_card' ? { kind: 'credit_card' as const, ...(funding.cardId === undefined ? {} : { cardId: requiredString(funding.cardId, 'payment route funding.cardId', true) }) } : fundingKind === 'account' && (subtype === 'linked_bank_account' || subtype === 'wallet_balance') ? { kind: 'account' as const, subtype: subtype as 'linked_bank_account' | 'wallet_balance' } : fundingKind === 'cash' && subtype === undefined && funding.cardId === undefined ? { kind: 'cash' as const } : (() => { throw new RewardServiceError('INVALID_INPUT', 'payment route funding kind or subtype is invalid'); })();
