...
export function validateIngestionBenefitLeaf(value: unknown): IngestionBenefitLeafSubmission {
  const item = object(value, 'benefit leaf submission'); keys(item, ['flowId', 'actionId', 'expectedRevision', 'idempotencyKey', 'leafId', 'offer', 'merchantRefs', 'evidenceRefs', 'localExclusions'], 'benefit leaf submission');
  ...
  return { flowId: requiredString(item.flowId, 'benefit leaf flowId', true), actionId: requiredString(item.actionId, 'benefit leaf actionId', true), expectedRevision: item.expectedRevision as number, idempotencyKey: requiredString(item.idempotencyKey, 'benefit leaf idempotencyKey', true), leafId: requiredString(item.leafId, 'benefit leaf leafId', true), offer: normalizedOffer, ...(merchantRefs ? { merchantRefs } : {}), evidenceRefs: item.evidenceRefs.map((v, i) => requiredString(v, `benefit leaf evidenceRefs[${i}]`)), ...(localExclusions ? { localExclusions } : {}) };
}
