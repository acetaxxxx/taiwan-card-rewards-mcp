...
export function validateToolArgs(name: string, value: unknown): Record<string, unknown> {
  ...
  if (name === 'remaining_caps' && normalized.includeHistorical !== undefined && typeof normalized.includeHistorical !== 'boolean') {
    throw new RewardServiceError('INVALID_INPUT', 'includeHistorical must be a boolean');
  }
  return normalized;
}
