...
function validatePaymentRouteContext(value: unknown): PaymentRouteContext {
  ...
  if (owner !== undefined && !['merchant', 'wallet', 'payment_provider', 'card_network', 'issuer', 'bank', 'acquirer', 'unknown'].includes(owner)) throw new RewardServiceError('INVALID_INPUT', 'transaction.routeContext.conversionOwner is invalid');
...
export function validateRewardBreakdown(value: unknown): RewardBreakdown {
  ...
  const diagnostics = item.diagnostics === undefined ? undefined : (Array.isArray(item.diagnostics) ? item.diagnostics.map((value, index) => { const d = object(value, `reward.diagnostics[${index}]`); keys(d, ['code', 'path', 'requiredFacts', 'retryAction'], 'reward diagnostic'); if (!Array.isArray(d.requiredFacts) || d.requiredFacts.some((fact) => typeof fact !== 'string')) throw new RewardServiceError('INVALID_INPUT', 'reward diagnostic requiredFacts must be strings'); return { code: requiredString(d.code, 'reward diagnostic code') as never, path: requiredString(d.path, 'reward diagnostic path'), requiredFacts: [...d.requiredFacts], retryAction: requiredString(d.retryAction, 'reward diagnostic retryAction') }; }) : (() => { throw new RewardServiceError('INVALID_INPUT', 'reward.diagnostics must be an array'); })());
...
export function validateStoredState(value: unknown): StoredState {
  ...
  keys(item, ['schemaVersion', 'cards', 'snapshots', 'rules', 'transactions', 'campaigns', 'switchEnrollments', 'cardSwitches', 'capPools', 'rewardComponents', 'merchants', 'evidence', 'factCandidates', 'paymentRoutes', 'paymentAccounts', 'eventRewardSchemaVersion', 'eventRewardLedger', 'eventRewardReversals', 'eventRewardCapUsage'], 'stored state');
...
export function validatePaymentRouteRecord(value: unknown): PaymentRouteRecord {
  ...
  const status = requiredString(item.status, 'payment route status'); if (!['candidate', 'active', 'stale', 'conflict', 'needs_review'].includes(status)) throw new RewardServiceError('INVALID_INPUT', 'payment route status is invalid');
  ...
  const edges = item.edges === undefined ? undefined : (Array.isArray(item.edges) ? item.edges.map((value, index) => { const edge = object(value, `payment route edges[${index}]`); keys(edge, ['edgeId', 'fromNodeId', 'toNodeId', 'transition', 'evidenceIds', 'provenance', 'market', 'currency', 'validFrom', 'validTo'], 'payment route edge'); const transition = requiredString(edge.transition, 'payment route edge.transition'); if (!['card_authorization', 'account_debit', 'wallet_top_up', 'wallet_debit', 'service_to_acceptance', 'merchant_settlement', 'direct_settlement', 'split_tender'].includes(transition)) throw new RewardServiceError('INVALID_INPUT', 'payment route edge transition is invalid'); const edgeEvidence = Array.isArray(edge.evidenceIds) ? edge.evidenceIds.map((id) => requiredString(id, 'payment route edge.evidenceIds[]', true)) : (() => { throw new RewardServiceError('INVALID_INPUT', 'payment route edge evidenceIds must be an array'); })(); return { edgeId: requiredString(edge.edgeId, 'payment route edge.edgeId', true), fromNodeId: requiredString(edge.fromNodeId, 'payment route edge.fromNodeId', true), toNodeId: requiredString(edge.toNodeId, 'payment route edge.toNodeId', true), transition: transition as any, evidenceIds: edgeEvidence, ...(edge.provenance === undefined ? {} : { provenance: requiredString(edge.provenance, 'payment route edge.provenance') as 'official' | 'model_fixture' }), ...(edge.market === undefined ? {} : { market: requiredString(edge.market, 'payment route edge.market') }), ...(edge.currency === undefined ? {} : { currency: requiredString(edge.currency, 'payment route edge.currency') }), ...(edge.validFrom === undefined ? {} : { validFrom: iso(edge.validFrom, 'payment route edge.validFrom') }), ...(edge.validTo === undefined ? {} : { validTo: iso(edge.validTo, 'payment route edge.validTo') }) }; }) : (() => { throw new RewardServiceError('INVALID_INPUT', 'payment route edges must be an array'); })());
  return { id: requiredString(item.id, 'payment route id', true), status: status as PaymentRouteRecord['status'], layers, funding: normalizedFunding, observedAt: iso(item.observedAt, 'payment route observedAt'), idempotencyKey: requiredString(item.idempotencyKey, 'payment route idempotencyKey'), ...(item.sourceUrl === undefined ? {} : { sourceUrl: requiredString(item.sourceUrl, 'payment route sourceUrl') }), ...(item.sourceSnapshotId === undefined ? {} : { sourceSnapshotId: requiredString(item.sourceSnapshotId, 'payment route sourceSnapshotId', true) }), ...(item.contentHash === undefined ? {} : { contentHash: requiredString(item.contentHash, 'payment route contentHash') }), ...(item.validFrom === undefined ? {} : { validFrom: iso(item.validFrom, 'payment route validFrom') }), ...(item.validTo === undefined ? {} : { validTo: iso(item.validTo, 'payment route validTo') }), ...(authority === undefined ? {} : { authority: authority as Exclude<PaymentRouteRecord['authority'], undefined> }), ...(confidence === undefined ? {} : { confidence: confidence as Exclude<PaymentRouteRecord['confidence'], undefined> }), ...(confirmation === undefined ? {} : { confirmation }), ...(evidenceIds === undefined ? {} : { evidenceIds }), ...(item.ownerUser === undefined ? {} : { ownerUser: requiredString(item.ownerUser, 'payment route ownerUser') }), ...(nodes === undefined ? {} : { nodes }), ...(edges === undefined ? {} : { edges }) };
}

export function validatePaymentAccountRecord(value: unknown): PaymentAccountRecord {
  const item = object(value, 'payment account');
  keys(item, ['id', 'providerId', 'kind', 'displayName', 'status', 'observedAt', 'sourceUrl', 'sourceSnapshotId', 'evidenceIds', 'confirmation', 'idempotencyKey', 'ownerUser'], 'payment account');
...
export function validateEvidence(value: unknown): EvidenceRecord {
  ...
  keys(item, ['id', 'requirementId', 'sourceIdentity', 'sourceType', 'authority', 'claim', 'observedAt', 'validFrom', 'validTo', 'refreshAfter', 'confidence', 'contentHash', 'reviewState', 'sourceUrl'], 'evidence');
