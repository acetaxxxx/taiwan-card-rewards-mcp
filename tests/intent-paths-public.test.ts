...
describe('public merchant intent path comparison', () => {
  it('returns direct card, wallet, and account candidates with layered rule terms', async () => {
    ...
    try {
      ...
      expect(result.candidates.filter((candidate: any) => candidate.kind === 'payment_path').map((candidate: any) => candidate.netSpend)).toEqual([
        { amountMinor: 9920, currency: 'TWD' },
        { amountMinor: 9940, currency: 'TWD' },
      ]);
