...
Available tools are the complete 8-tool MCP surface: `calculate_reward`, `rank_cards`, `register_card`, `list_cards`, `upsert_offer`, `recommend`, `record_transaction`, and `remaining_caps`. For comprehensive workflow instructions, see the [AI Agent Usage Guide](docs/agents/usage-guide.md). Phase 1 recommendation and cap usage read only actual transactions; planned transactions never mutate the store. Repeated actual calls require the same idempotency key and payload; mismatches fail closed. Refunds must reference an existing recorded transaction.
