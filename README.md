...
The executable must require `--data-dir <absolute-existing-directory>`. Startup canonicalizes it with the filesystem real path, rejects missing/non-directory paths and filesystem roots, and rejects unknown path-like arguments. Trusted public source hosts are supplied by the parent as repeated `--source-host <official-host>` arguments; fetches fail closed for any other host. This canonical directory is the sole tenant boundary and must be passed to every persistence operation; tools cannot override it. `--user`, if accepted, is display metadata only and never an authorization or storage selector. Reusing one `data-dir` is an explicit decision to share its data. Never store PAN, CVV, OTP, bank credentials, or provider keys.
